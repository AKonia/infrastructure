...
GraphMatIdent::GraphMatIdent(uint32_t countVertices, uint32_t countEdges)
{
    ...
    for(uint32_t i = 0; i < countVertices; ++i)
        for(uint32_t j = 0; j < countEdges; ++j)
            idenMat[i][j] = 0;
}
...
void GraphMatIdent::setEdge(uint32_t vFirst, uint32_t vSecond, uint32_t eNumber, uint32_t eWeight)
{
...
uint32_t GraphMatIdent::getEdgesSize() const
{
    return countEdges;
}

uint32_t GraphMatIdent::getVerticesSize() const
{
