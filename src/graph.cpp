...
Graph::Graph(uint32_t size) {
  ...
  for (uint32_t i = 0; i < size; ++i) connMat[i] = new uint32_t[size];
  for (uint32_t i = 0; i < size; ++i)
    for (uint32_t j = 0; j < size; ++j) connMat[i][j] = 0;
}
...
Graph::Graph(const Graph &rhs) {
  ...
  for (uint32_t i = 0; i < size; ++i) connMat[i] = new uint32_t[size];
  for (uint32_t i = 0; i < size; ++i)
    for (uint32_t j = 0; j < size; ++j) connMat[i][j] = rhs.connMat[i][j];
}

Graph::~Graph() {
  for (uint32_t i = 0; i < size; ++i) delete[] connMat[i];
  delete[] connMat;
}
...
uint32_t Graph::getSize() const { return size; }
